...
SELECT
   category_final,
   mapped_channel AS "Channel",  -- Fetch mapped_channel directly from the subquery
   CASE
   ...
   END AS "Range",
   ROUND(COALESCE(SUM(sellingprice), 0)) AS "Sales"
FROM (
   SELECT
       CASE
       ...
       cm.mapped_channel,  -- Use mapped_channel here and avoid second join
   ...
   LEFT JOIN dbt_dsarkar.channel_mapping_sheet cm
       ON usd.channelname = cm.channelname  -- Keep only this join
